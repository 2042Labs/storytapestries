			...
			if(!$is_siteurl_correct)
			{
				...
				elseif( strcmp($_POST['siteurl'], $_POST['siteurl_old']) != 0 ){
					...
					foreach($datas as &$meta_row){
						...
						$meta_serialized = serialize($meta_arr);
						$sql = "UPDATE $wpdb->postmeta SET meta_value = '$meta_serialized' WHERE meta_id = $meta_row->meta_id";
						$wpdb->query($sql);
					}
...
<body>
	...
	<div id="content">
		...
		<?php elseif ( !empty($messages) ) : ?>
			<p class="messages">- <?php echo implode('<br /><br />- ', $messages); ?></p>
		<?php endif; ?>
		<form action="" method="post">
			<fieldset>
				<?php if (!$is_siteurl_correct) : ?>
					<input type="hidden" name="siteurl_old" value="<?php echo $settings['siteurl'] ?>" />

					<label for="siteurl">Enter Your blog site url</label><br />
					<input id="siteurl" type="text" name="siteurl" size="40" value="<?php echo $base ?>" class="text" /><br />
					<span>current url is: <b><?php echo $settings['siteurl'] ?></b></span><br /><br />
				<?php endif; ?>
				<label for="siteurl">Change admin email: </label><br />
				<input id="siteurl" type="text" name="admin_email" size="40" value="<?php echo $settings['admin_email'] ?>" class="text" /><br /><br />

				<label for="siteurl">Change upload directory: </label><br />
				<input id="siteurl" type="text" name="upload_path" size="40" value="<?php echo $_SERVER['DOCUMENT_ROOT'].$base_path.'/wp-content/uploads' ?>" class="text" /><br />
				<span>current dir is: <b><?php echo $settings['upload_path'] ?></b></span>
