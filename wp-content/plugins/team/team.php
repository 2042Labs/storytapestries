...
function team_activation()
	{
		load_plugin_textdomain('team', false, basename( dirname( __FILE__ ) ) . '/languages' );
		
		
		$team_version['current'] = "1.5";
		
		if(is_array(get_option( 'team_version' )))
			{
				$team_version =  get_option( 'team_version' );
			}
		else
			{
				$team_version['previous'] =  get_option( 'team_version' );
			}
		...
		$team_customer_type= "free"; //customer_type "pro"
...
function team_menu_update(){
	include('team-update.php');	
}
...
function team_menu_init()
	{
		...
		add_submenu_page('edit.php?post_type=team', __('Settings','team'), __('Settings','team'), 'manage_options', 'team_menu_settings', 'team_menu_settings');
		...
		if($team_version_ok == 'ok')
			...
			{
		add_submenu_page('edit.php?post_type=team', __('Update','team'), __('Update','team'), 'manage_options', 'team_menu_update', 'team_menu_update');
			}
